...
def get_neighbors(indices, query_id):
    ...
    print(df.loc[top10_neighbor_ids, ["uqid", "sent"]])
    # df.loc[top10_neighbor_ids, ["uqid", "sent"]].to_csv(word+'-'+str(layer_number)+'.csv')
...
fig.show()
fig.write_image("./plot/" + word + "-" + str(layer_number) + ".png")
fig.write_html("./plot/" + word + "-" + str(layer_number) + ".html")
...
uqid_to_dfid = dict(zip(df.uqid, df.index))
uqid_to_intid = dict(zip(df.uqid, zip(range(len(X)))))
...
# Aggregate neighboring sentences (used for BERT embedding)
df['3sents'] = df['prev'] + ' ' + df['sent'] + ' ' + df['next']

df_centroids = df.iloc[nearest_to_centroids]
print('Centroids:')
print(df_centroids[['uqid', '3sents']])
...
# Inspection of each data
# query_id = 138
# get_neighbors(indices_mapped, query_id)
# get_neighbors(indices_mapped, uqid_to_dfid[2689088])
